var UserController = function (userModel) {
  ...
  var readAllUsers = function (callback) {
    userModel.find(function (err, users) {
      ...
      return callback(
        ...
        new ApiResponse({
          success: true,
          extras: { userProfileModels: userProfileModels },
        })
      );
    });
  };
    ...
    userModel.findById(id, function (err, user) {
      ...
      if (user) {
        ...
        return callback(
          ...
          new ApiResponse({
            success: false,
            extras: { msg: ApiMessages.NOT_FOUND },
          })
        );
      }
    });
  ...
  var createUser = function (user, callback) {
    ...
    hashPassword(user.password, user.passwordSalt, function (
      err,
      passwordHash
    ) {
      if (err) {
  ...
  var userIsValid = function (email, password, callback) {
    userModel.findOne({ email: email }, function (err, user) {
      ...
      hashPassword(password, userPasswordSalt, function (
        err,
        derivedPasswordHash
      ) {
        if (err) {
