...
export function drawRoundRectWPoint(
  ctx,
  x,
  y,
  w,
...
export function draw() {
  if (validCanvas == false) {
    ...
    printAtWordWrap(
      context,
      levelControl.getCurrentLevel().toString(),
      330,
      20,
      10,
      30,
...
export function drawButtons(ctx, buttons) {
  ...
  for (let button of buttons) {
    ...
    printAtWordWrap(
      ctx,
      button.text,
      button.x + button.w / 2 + 3,
      button.y + button.h / 1.5,
      button.h,
      button.w,
      "black",
      button.font,
...
export function drawRollPieCuts(context, x, y, radius, roll) {
  switch (roll.inner.length) {
    default:
      drawPieCuts(context, x, y, radius * 0.94, roll);
      break;
    case 1:
